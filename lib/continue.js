...
var depset = function(v, key, value) {
  var ss = key.split('.');
...
var Hash = function() {
  return {__proto__: Hash_proto};
};
Hash.prototype = Hash_proto;
...
var C_proto = {
  ...
  constructor: function(node, prev_c) {
    ...
    if (arguments.length === 2) {
      ...
      obj.node = node;
      obj.next = node.next;
      obj.locals = prev_c.locals;
      obj.lastErr = prev_c.err;
  ...
  get: function(key) {
    ...
    if (key[0] === '$') {
      return depget(this, key.slice(1));
    } else {
      return depget(this.locals, key);
    }
  },
  set: function(key, value) {
    ...
    if (key[0] === '$') {
      depset(this, key.slice(1), value);
    } else {
      depset(this.locals, key, value);
    }
  },
  assign: function() {
    var keys = arguments;
  ...
  assign2: function() {
    var keys_pair = arguments;
    var c = this;
    var func = function(args) {
      for (var i = 0; i < keys_pair.length / 2; ++i) {
        if (keys_pair[i * 2] && keys_pair[i * 2 + 1]) {
          depset(keys_pair[i * 2], keys_pair[i * 2 + 1], args[i]);
        }
      }
      c.apply(c, args);
    };
    var obj = func_length_n(keys_pair.length / 2, func);
    obj.__proto__ = this;
    return obj;
  }
};
...
var Block_proto = {
  ...
  invoke_callback: function(c, func) {
    if (func.length <= 1 || c.args.length === 0) {
      return func.apply(c.locals, [c]);
    } else {
      var args = [c].concat(Array.prototype.slice.call(c.args));
      return func.apply(c.locals, args);
    }
  },
  ...
  parallel_wrap: function(prev_c, limit, iter, args, catch_break) {
    ...
    var spawn = function() {
      ...
      c.next = {invoke: next};
      c.looping = true;
      var func = iter.apply(c, args);
      if (func) {
        ncur++;
        nspawn++;
        node.exception_protect(prev_c, func, c);
        return true;
      } else {
        return false;
      }
  ...
  toPromise: function() {
    ...
    return new Promise(function(resolve, reject) {
      this_node.last(function(c, locals) {
        if (c.lastErr) {
          reject(c.lastErr);
        } else {
